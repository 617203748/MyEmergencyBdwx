...
public class Message {
	 ...
	 **/
	public CodeSuccessReslut publishPrivate(String fromUserId, List<String> toUserId, BaseMessage message, String pushContent, String pushData, String count, Integer verifyBlacklist, Integer isPersisted, Integer isCounted) throws Exception {
		if (fromUserId == null) {
	 ...
	 *            :发送消息内容（必传）
	 * @param pushContent
	 *            :如果为自定义消息，定义显示的 Push 内容，内容中定义标识通过 values 中设置的标识位内容进行替换.如消息类型为自定义不需要 Push 通知，则对应数组传空值即可。（可选）
	 * @param pushData
	 ...
	 *            :发送人用户 Id。（必传）
	 * @param toChatroomId
	 *            :接收聊天室Id，提供多个本参数可以实现向多个聊天室发送消息。（必传）
	 * @param txtMessage
	 *            :发送消息内容（必传）
	 * 
	 * @return CodeSuccessReslut
	 ...
	 * 
	 * @param date
	 *            :指定北京时间某天某小时，格式为2014010101,表示：2014年1月1日凌晨1点。（必传）
	 * 
